...
export default function WordToPdfPage() {
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (dropped) { setFile(dropped); setConverted(false); }
  }, []);
  ...
  const handleConvert = () => {
    if (!file) return;
    setConverting(true);
    setTimeout(() => { setConverting(false); setConverted(true); }, 2000);
  };
              ...
              >
                ...
                {file ? (
                  <div className="flex flex-col items-center gap-3 p-6">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); setFile(null); setConverted(false); }}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-emerald-600 dark:text-emerald-400">
                      {file?.name.replace(/\.docx?$/, ".pdf")} · Ready to download
                    </p>
                  </div>
                </div>
