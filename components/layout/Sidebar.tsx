      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-4 border-b border-slate-700 shrink-0">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="text-lg font-bold bg-gradient-to-r from-blue-400 via-purple-400 to-cyan-400 bg-clip-text text-transparent animate-gradient"
              style={{ backgroundSize: "200% 200%" }}>
              ConvertAI
            </span>
          </Link>
          <button onClick={onClose} className="md:hidden text-slate-400 hover:text-white p-1 rounded">
            <X size={18} />
          </button>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-3 space-y-4 scrollbar-thin">
          {navItems.map((group) => (
            <div key={group.section}>
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider px-2 mb-1.5">
                {group.section}
              </p>
                  ...
                  return (
                    <li key={item.href}>
                      <Link href={item.href} onClick={onClose}
                        ...
                        {active && <ChevronRight size={13} className="ml-auto opacity-70 shrink-0" />}
                      </Link>
                    </li>
                  );
                })}
          ...
          <div>
            ...
            <Link href="/admin" onClick={onClose}
              ...
              {pathname === "/admin" && <ChevronRight size={13} className="ml-auto opacity-70 shrink-0" />}
            </Link>
          </div>
        </nav>

        {/* User + dark mode */}
        <div className="px-3 pb-3 border-t border-slate-700 pt-3 shrink-0">
          <div className="flex items-center gap-3 mb-2.5">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-white truncate">Aditya Singh</p>
              <p className="text-xs text-slate-500 truncate">adityasingh018adi@gmail.com</p>
            </div>
          </div>
